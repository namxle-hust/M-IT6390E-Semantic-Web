...
class WikipediaCollector:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"All API request attempts failed for params: {params}")
                    return None
    ...
    def get_articles_from_category(self, category: str, limit: int = 50) -> List[WikipediaArticle]:
    ...
    def collect_articles_by_categories(self, max_per_category: int = 20) -> List[WikipediaArticle]:
    ...
    def save_articles_to_json(self, articles: List[WikipediaArticle], output_path: str) -> None:
    ...
    def get_collection_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'success_rate': (len(self.collected_articles) / 
                           (len(self.collected_articles) + len(self.failed_articles))) * 100,
            'articles_with_infobox': sum(1 for article in self.collected_articles.values() 
                                       if article.infobox),
            'average_categories_per_article': sum(len(article.categories) 
        ...
        for article in self.collected_articles.values():
            if article.infobox and 'template_type' in article.infobox:
                ...
                stats['infobox_templates'][template] = stats['infobox_templates'].get(template, 0) + 1
